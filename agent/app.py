...
from agent.approvals import (
    AgentApprovals, ApprovalError, validate_agent_docker_target, validate_agent_patch_id,
)
from agent import prompts as agent_prompts
from agent.prompts import normalize_json_structural_whitespace, parse_agent_json
from agent.evidence import (
    FILE_EXCERPT_MAX_CHARS,
    normalize_agent_conversation_context,
    compact_agent_observations,
    ambiguous_delete_reference,
    coding_read_only_fast_final_requested,
    orchestrator_required_evidence,
    orchestrator_observed_evidence,
    orchestrator_missing_evidence,
    normalize_delegate_goal,
    orchestrator_delegate_attempts,
    orchestrator_duplicate_delegation,
    orchestrator_unresolved_subagent_quality,
    coding_evidence_contract,
    coding_final_answer_requires_repair,
    compact_coding_inline_paragraphs,
    compact_coding_answer_markdown,
    boost_orchestrator_research_query,
    sanitize_orchestrator_web_query,
    source_is_primary_for_goal,
    deterministic_loaded_web_sources,
    research_goal_requires_primary_source,
    assess_subagent_report,
    canonical_github_repo_from_goal,
    degraded_empty_web_search_count,
)
...
def agent_llm(messages, max_tokens=1200, temperature=0.1):
    ...
    response = agent_model_provider().complete(
        request, run_context=run_state.current_run_context(),
    )
...
def agent_choose_next_step_v2(goal, observations, max_steps=MAX_TOOL_STEPS,
    ...
    return agent_prompts.agent_choose_next_step_v2(
        ...
        agent_tool_description=agent_tool_description,
        _looks_like_disk_usage_request=_looks_like_disk_usage_request,
    )




def agent_approvals(runtime=None):
    return AgentApprovals(
        runtime.registry if runtime is not None else AGENT_TOOL_REGISTRY,
        PENDING_AGENT_ACTIONS, PENDING_AGENT_ACTIONS_LOCK, AGENT_APPROVAL_TTL,
    )


def agent_action_permission(operation, target, context):
    return agent_approvals().permission(operation, target, context)


def create_agent_approval(goal, observations, step, mode, operation, target, reason,
                          conversation_context=None, *, runtime=None,
                          progress_callback=None, tool_request=None):
    return agent_approvals(runtime).create(
        goal, observations, step, mode, operation, target, reason,
        conversation_context, runtime=runtime, progress_callback=progress_callback,
        tool_request=tool_request,
    )


def execute_agent_approved_action(pending):
    return agent_approvals(pending.get("runtime")).execute(pending)


def agent_coding_read_only_final_answer(goal, observations):
    return agent_prompts.agent_coding_read_only_final_answer(
        goal, observations, observed_agent_llm=observed_agent_llm,
    )


def agent_v2_final_answer(goal, observations):
    return agent_prompts.agent_v2_final_answer(
        goal, observations, observed_agent_llm=observed_agent_llm,
    )


def build_subagent_report(agent_name, goal, sub_result):
    return agent_prompts.build_subagent_report(
        agent_name, goal, sub_result, observed_agent_llm=observed_agent_llm,
    )


def agent_runtime(context, *, provider=None, registry=None, policy=None):
    hooks = RuntimeHooks(
        coding_read_only_fast_final_requested=coding_read_only_fast_final_requested,
        ambiguous_delete_reference=ambiguous_delete_reference,
        agent_choose_next_step_v2=agent_choose_next_step_v2,
        agent_coding_read_only_final_answer=agent_coding_read_only_final_answer,
        agent_v2_final_answer=agent_v2_final_answer,
        orchestrator_missing_evidence=orchestrator_missing_evidence,
        orchestrator_unresolved_subagent_quality=orchestrator_unresolved_subagent_quality,
        orchestrator_delegate_attempts=orchestrator_delegate_attempts,
        orchestrator_duplicate_delegation=orchestrator_duplicate_delegation,
        create_agent_approval=create_agent_approval,
        execute_agent_approved_action=execute_agent_approved_action,
        verify_agent_action=verify_agent_action,
        build_subagent_report=build_subagent_report,
        assess_subagent_report=assess_subagent_report,
        compact_agent_observations=compact_agent_observations,
        sanitize_orchestrator_web_query=sanitize_orchestrator_web_query,
        boost_orchestrator_research_query=boost_orchestrator_research_query,
        degraded_empty_web_search_count=degraded_empty_web_search_count,
        canonical_github_repo_from_goal=canonical_github_repo_from_goal,
    )
    return AgentRuntime(
        context, provider if provider is not None else agent_model_provider(),
        registry if registry is not None else AGENT_TOOL_REGISTRY,
        hooks, policy=policy, logger=logger,
    )


@run_state.with_run_context
def run_agent_v2(
    goal, observations=None, start_step=1, mode="diagnostic",
    conversation_context=None, progress_callback=None, allow_approval=True,
    *, run_context=None,
):
    if not str(goal or "").strip():
        raise HTTPException(status_code=400, detail="Kein Agent-Ziel angegeben")
    runtime = agent_runtime(run_state.current_run_context())
    return runtime.run(
...
def verify_agent_action(pending):
    return agent_approvals(pending.get("runtime")).verify(pending)


@app.post("/api/agent/approve/{approval_id}")
def api_agent_approve(approval_id: str, request: AgentApprovalRequest):
    try:
        pending = agent_approvals().take(approval_id)
...
def _resume_agent_approval(pending, request):
    ...
    runtime = pending.get("runtime") or agent_runtime(context)
    return runtime.resume_approval(pending, request.approved)
